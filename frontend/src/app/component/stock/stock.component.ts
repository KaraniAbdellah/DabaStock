...
export class StockComponent {
  ...
  user_id_iden: string = `${
    document.cookie
      .split('; ')
      .find((c) => c?.startsWith('user_id_iden='))
      ?.split('=')[1] || ''
  }`;
  ...
  add_product() {
    if (
      ...
      if (this.form_state == 'Create Product') {
        ...
        setTimeout(() => {
          this.getProducts(this.user_id_iden);
        }, 2000);
      } else {
        ...
        this.productService
          .updateProduct(this.current_product_id, updatedProduct)
          .subscribe({
            next: (response) => {
  ...
  getProducts(user_id_iden: string) {
    this.productService.getAllProducts(user_id_iden).subscribe((data: any) => {
      this.products = data;
